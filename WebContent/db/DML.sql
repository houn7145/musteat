...
SELECT * FROM(SELECT ROWNUM RN, A.* FROM (SELECT * FROM RESTAURANT WHERE RPLACE LIKE '%' || '서울' || '%')A)
    WHERE RN BETWEEN 1 AND 3;
    
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT RESTAURANT.*, (SELECT AVG(AVG) FROM RAVG WHERE RNO=RESTAURANT.RNO)RAVG FROM RESTAURANT WHERE RPLACE LIKE '%' || '구' || '%' ORDER BY RAVG) A)
...
-- 4-1. 카테고리 별 맛집 출력(평점도 같이 출력) 
SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT RESTAURANT.*, (SELECT AVG(AVG) FROM RAVG WHERE RNO=RESTAURANT.RNO)RAVG FROM RESTAURANT WHERE CNO = '1' ORDER BY RAVG) A)
 WHERE RN BETWEEN 1 AND 2;
