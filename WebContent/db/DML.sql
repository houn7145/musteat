...
-- 9. 5명 이상 평점을 등록한 맛집 중 평점이 4 이상인 맛집 출력

SELECT RESTAURANT.*, C.CNAME, RAVG.RAVG FROM RESTAURANT, CCODE C, (SELECT R.RNAME, AVG(RATING)RAVG FROM RAVG, RESTAURANT R WHERE RAVG.RNO = R.RNO GROUP BY R.RNAME HAVING AVG(RATING) > 4)RAVG WHERE RESTAURANT.RNAME = RAVG.RNAME AND RESTAURANT.AVGHIT >= 5 AND RESTAURANT.CNO = C.CNO ORDER BY RAVG DESC;

-- 10. 맛집 지역으로 검색 

SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT RESTAURANT.*, (SELECT NVL(AVG(RATING),0) FROM RAVG WHERE RNO=RESTAURANT.RNO )RAVG FROM RESTAURANT WHERE RPLACE LIKE '%' || '서울' || '%' ORDER BY RAVG DESC) A)WHERE RN BETWEEN 1 AND 5;
--------------------------------
----------------- OneReview
--------------------------------
